...
Memory is divided into active context + long-term knowledge, with core system-prompt memories represented as flagged memories with MemIds.

### 2.1 Active Memory (Working Context)
...
*   **Data Integrity:** Every object in active memory has a unique `MemoryID`.
...
### 2.3 Core + Identity Files (Personality Layer)
Each agent is defined by core markdown templates that dictate behavior:
1.  `CORE.md`: Base runtime instructions, including how to interpret active memory fields.
2.  `SOUL.md`: Core values, ethical boundaries, and communication tone.
3.  `AGENT.md`: Operational workflow rules and "contract" (how it handles tasks).
4.  `IDENTITY.md`: Public-facing avatar, name, and role description.
...
*   **Memory Manager:** Continuously optimizes active memory by summarizing old context and moving details to LTM to maintain token efficiency.
...
With both the Primary Brain and the Memory Manager (and potentially other Sub-Brains) able to `UPDATE` or `EVICT` active-memory data, there is a risk of context "hallucination" or inconsistency.
*   **Risk:** The Primary Brain might refer to a `MemoryID` that the Memory Manager just evicted or summarized mid-thought.
*   **Recommendation:** Implement a simple locking mechanism or a "Proposed Changes" queue for active-memory updates.
...
*   **Recommendation:** Define "Soft Workflows" (as seen in `ARCHITECTURE.md`) where sub-brains can update shared active memory directly for routine tasks without bothering the Primary Brain.

### 4. MemoryID Discovery
*   **Issue:** How does a brain "know" which `MemoryID` to look up if it has been evicted?
*   **Recommendation:** The Memory Manager should leave "Tombstones" or "Summaries" in active memory that contain the `MemoryID` for the full detail, allowing the Primary Brain to trigger a "Recall" action.
